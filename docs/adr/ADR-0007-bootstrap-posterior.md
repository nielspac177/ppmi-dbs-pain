...
- Status: **Provisional** (peer reviewer has objected — see Sprint 14 follow-up)
- Date: 2026-05-19

## Context

Sprint 11 attempts a Bayesian re-analysis of the null genetic / biomarker
...
Implement Sprint 11 as an **empirical bootstrap posterior
approximation**:
...
- **Install `brms`/`rstanarm` in the container.** Will be done in
  Sprint 14. Requires updating `Dockerfile` and `renv.lock`.
...
the posterior framing or genuinely fit `brms`. Tracked as Sprint 14
