...
emulation + numbered-analysis framework established in the pain paper.
...
- Builds on the late-post network reorganisation finding (Analysis 04).
...
- Analysis 01-equivalent: NP1HALL, NP1COG as negative controls.
- Analysis 02-equivalent: 3 anchor sweep.
- Analysis 04-equivalent: NCT on 15-node network with sleep-focused
  pain neighbourhood.
- Analysis 09-equivalent: ΔLEDD mediation; expected positive (unlike pain).
...
after DBS** (replicating the Analysis 04 finding) — specifically the
...
- Analysis 01-equivalent: NP1HALL, NP1COG as negatives; UPDRS-III as
  positive.
- Analysis 04-equivalent: SCOPA item-level GLASSO; NCT on autonomic
  sub-network.
- Analysis 08-equivalent: Fine-Gray for time to incident orthostatic
  hypotension.
...
- Analysis 01-equivalent: NP1PAIN, NP1HALL as negatives (MoCA-irrelevant);
  UPDRS-III as positive.
- Analysis 04-equivalent: cognitive sub-network (MoCA, LNS, HVLT,
  semantic fluency, Symbol Digit) with arm stratification.
- Analysis 08-equivalent: dementia-free survival.
- Analysis 11-equivalent: Bayesian APOE × DBS posterior on ΔMoCA.
...
- The 16-analysis robustness convention (`MANIFEST.md` + ADR-0003).
- The Tailwind + Plotly dashboard (`scripts/build_dashboard.py` is
  generic — point it at a new `outputs/tables/NN_*.csv` set).
